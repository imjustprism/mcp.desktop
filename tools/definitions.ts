...
export const TOOLS: MCPTool[] = [
    ...
    },
    {
        name: "store",
        description:
            "Flux stores. Auto-resolves 'User'→'UserStore'. list: all names. find: methods/getters/props. methods: prototype chain. state: get value. call: invoke method. subscriptions: handled events.",
        inputSchema: {
            type: "object",
            properties: {
                action: {
    ...
    },
    {
        name: "intl",
        description:
            "Internationalization. hash: key→hash+message. reverse: hash→key+message. search: by message text. scan: extract hashes from module. targets: modules using key. bruteforce: with hash=single hash check, without hash=batch all unknowns (returns results directly with anchor positions). test: try custom candidates/combinations against hash(es). unknown: list hashes NOT in keymap. Use #{intl::KEY} in patches.",
        inputSchema: {
            type: "object",
            properties: {
                action: {
                    type: "string",
                    enum: ["hash", "reverse", "search", "scan", "targets", "bruteforce", "test", "unknown"],
                    description: "Action to perform",
                },
                key: {
                    type: "string",
        ...
        inputSchema: {
            ...
            properties: {
                ...
                finders: {
                    type: "array",
                    description:
                        "Array of webpack finder specs to validate (for finds action). Each: {type: 'byProps'|'byCode'|'store'|'componentByCode'|'exportedComponent'|'cssClasses', args: string[], plugin?: string}",
                    items: {
                        type: "object",
                        properties: {
                            type: {
    ...
    },
    {
        name: "react",
        description:
            "React/DOM. query: find elements. fiber: component tree. props/state/hooks: component data. contexts: providers. find: by component name/props. styles: computed CSS. tree: DOM subtree. modify: set styles/classes. text: content. path: CSS selector. forceUpdate: re-render. owner: parents. root: fiber stats.",
        inputSchema: {
            type: "object",
            properties: {
                action: {
    ...
    },
    {
        name: "testPatch",
        description:
            "Test patch before writing. Checks find uniqueness, applies match regex, validates captures, previews replacement. Shows canonicalized regex, match context, nearby anchors. Returns VALID/FIND_NOT_UNIQUE/MATCH_FAILED. benchmark: run replace in tight loop to measure speed.",
        inputSchema: {
            type: "object",
            properties: {
                find: {
    ...
    },
    {
        name: "trace",
        description:
            "Flux tracing. events: list all. handlers: stores for event. storeEvents: events for store. start: capture actions. get: retrieve. stop: end. store: watch state changes. Auto-expires.",
        inputSchema: {
            type: "object",
            properties: {
                action: {
    ...
    },
    {
        name: "intercept",
        description:
            "Intercept function calls. set: start capturing. get: retrieve args/returns. stop: restore. Captures args, returns, timestamps. Auto-expires. exportKey supports dotted paths (e.g. 'A.sendMessage').",
        inputSchema: {
            type: "object",
            properties: {
                action: {
