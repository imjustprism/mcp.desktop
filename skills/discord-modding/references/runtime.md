...
Tools for inspecting the *running* Discord renderer: the React tree, Flux stores, the dispatcher, live function calls, and the console. Use these when static source reading (search/module/graph) falls short. Use them to learn what a component actually rendered, what a store currently holds, or what fired when the user clicked.
...
- **Auto-expiry DISCARDS captures. Get before the window closes.** Once `duration` elapses, `trace.get`/`trace.stop` return `Trace N not found` (and `trace.store` watches return `Watch N not found or expired`). The captured actions are gone, not retained. Always `trace.get` inside the window. `trace.stop` while still in-window returns the final results. Same rule for `intercept` (below).
...
`duration` 1000 to 60000 ms. For static event/handler listing use `flux`/`store` instead. Trace is for the temporal picture.
...
```
`duration` 5000 to 120000 ms. It auto-expires and restores even if you forget to stop. Prefer `intercept` over `evaluateCode` monkey-patching. It cleans up after itself.
...
**Check `console.recent` after every `reloadDiscord` and after any patch change.** It confirms a patch didn't throw at module-eval or render time.
...
**Latency (plan your batches around it):** warm read-only calls are 30 to 300ms. But the **first `store`/`flux` call after startup pays a ~2s index-build cost** (warm thereafter). Fire one throwaway store/flux call early, or expect the first real one to be slow. **Patch registry scanners, `patch.broken` / `patch.conflicts` / `patch.analyze`, run 2 to 4.5s** (whole-registry walk). Keep them OUT of latency-sensitive batches, run them standalone.
