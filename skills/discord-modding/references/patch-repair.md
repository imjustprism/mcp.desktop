...
When a Discord update breaks a patch, there are two independent failure surfaces: the **find** (no longer locates a unique module) and the **match** (regex no longer fits the module's source). `patch.suggestFix` repairs both in one call; `patch.verifyApplied` proves the fix actually landed.
...
| `lookaround-stale` | The regex only fails because a lookahead/lookbehind asserts context that no longer exists around the anchor. | Yes — the stale lookaround(s) are stripped; verified `adjustedMatch` returned |
| `literals-missing` | One or more literal runs (>=4 chars) from the pattern no longer appear anywhere in the source. The code the pattern anchored on was renamed or removed. | No — rewrite by hand; `missingLiterals` tells you which anchors died |
...
Note `\i` in patterns is expanded to the minified-identifier class `(?:[A-Za-z_$][\w$]*)` before any matching, same as Vencord's canonicalizer.
...
Only bounded quantifiers of the exact form `{lo,hi}` whose atom is a single char/class are candidates — quantifiers attached to a group `(...){0,5}` or to `\i` are never touched, and patterns with more than **8** bounded quantifiers are not attempted.
...
Widened repairs return `adjustmentNote` like `"widened a bounded gap to the minimum that still matches"`. Because the result is re-minimized, the adjusted pattern stays tight — it will not silently swallow unrelated code. Real live result shape:
...
Lookarounds are removed cumulatively from last to first. A stripped candidate is only accepted if **all three** hold:
...
**Capture-safety guarantee.** If a stale lookaround being stripped *contains a capturing group*, repair refuses that candidate outright (`hasCapturingGroup` → returns `null`, `status:"unrepaired"`) — removing it would delete a `(...)` and shift every subsequent `$n` reference in the `replace`. An `adjustedMatch` therefore **never silently changes capture numbering**: you either get a repair that preserves all captures, or an unrepaired verdict you must fix by hand. Repairs that only strip zero-width `(?=...)`/`(?<=...)` assertions (no captures inside) are safe and proceed.

### Safety budgets (why repairs degrade instead of hang)

All numeric limits are hard caps: MAX_GAP 1000, quantifier count cap 8, single-gap step budget 200k, multi-gap budget 50M source-chars, literal-run minimum 4 chars for triage. When a budget is exceeded the repair simply reports `status: "unrepaired"` with the best `failureKind` it could determine — it never spins.
...
Discovery probes are **exact fragments** split on punctuation (`>=6` chars); it never fuzzy-matches. A stale find that is one unbroken token with a typo *inside* the token (e.g. a renamed method with no surrounding punctuation) produces zero usable probes → empty `targetCandidates`. Manual fallback chain:
...
Proves a plugin's patches actually took effect at runtime — run this after editing a patch and reloading.
...
- `ALL_APPLIED` — every patch is `APPLIED`. The patches landed. (Note: named `ALL_APPLIED`, *not* `ALL_APPLIED_CLEAN` — cleanliness of the console is reported separately, see below.)
