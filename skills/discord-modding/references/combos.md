...
You hold a module ID and want everything about it at once.
```json
...
```
then `testPatch`. **For visible UI copy, replace trip 1 with `intl.search{query}` → `intl.targets{key}`.** A literal `resolve` of UI text lands on the ~1.4MB locale bundle, a dead end, unpatchable by design.
...
```
API surface, handled actions plus dependsOn/dependents DAG, sync partners plus subscriber counts, live state, and module id. Note `snapshot` only invokes **zero-arg getters** (typically 2-3 even when `find` lists 20+ methods). `resolve` on a StoreName returns the module under `referencingModules` (empty `definingModules` is normal).
...
```
Captures are `{ts,type[,data]}` plus `typeCounts`. The action payload (everything but `type`) is included as `data` when non-empty. For a function's args and return values use the `intercept` tool instead. **Auto-expiry discards captures**, so always `get` before the window closes. `stop` inside the window returns final results. Reach for this when mapping which actions fire during a live interaction.
