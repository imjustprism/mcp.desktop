...
Dense lookup tables for mid-task use. All figures from a live Discord Canary eval (Equicord 1.14.15.2). Verify action/arg names against `tools/definitions.ts` before use.
...
| `PASS_WITH_WARNINGS` | find unique + match applied, non-fatal warnings present |
| `PASS_WITH_ERRORS` | find unique + match applied, error-severity warnings present (e.g. post-replace syntax error, invalid capture ref) |
| `FIND_NOT_UNIQUE` | find hit >1 module |
| `FIND_NO_MATCH` | find hit ZERO modules (also what you get if you pass a `/regex/`, since testPatch searches the find literally) |
...
| `gap-too-narrow` | a bounded gap `{0,N}` is too small, widen it |
...
| `unrepaired` | could not safely repair (e.g. refused to strip a stale lookaround containing a capturing group, a capture-safe guarantee) |
