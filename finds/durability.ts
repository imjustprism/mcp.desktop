...
export function scoreDurability(find: string): Durability {
    ...
    if (INTL_PLACEHOLDER_RE.test(find)) {
        ...
        reasons.push("anchored on an intl key (#{intl::KEY}), content-independent and stable per key name");
    ...
    if (HEX_HASH_RE.test(find) && !intlPlaceholder) {
        score -= 3;
        reasons.push("contains a 16+ char hex hash (CDN asset / content hash) that rotates when the asset or build changes");
    }
    ...
    if (tier === "weak" || tier === "string") {
        ...
        if (/^[A-Za-z][A-Za-z.,'!?-]*(?: [A-Za-z.,'!?-]+)+$/.test(inner) && !/[{}()[\];=:]/.test(find)) {
            score -= 1;
            reasons.push("plain English copy that rots when Discord edits the wording. Prefer the intl key if one exists");
        }
    }
